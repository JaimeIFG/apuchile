        ...
        {creando && (
          <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md mx-4">
              <h3 className="text-lg font-bold text-gray-800 mb-5">Nuevo proyecto</h3>
              <input autoFocus type="text" value={nombreNuevo} onChange={e => setNombreNuevo(e.target.value)}
                ...
                className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 mb-4"/>
              ...
              {proyectos.map(p => (
                <button key={p.id} onClick={() => abrirProyecto(p.id)}
                  ...
                  <div className="text-xs text-gray-400">
                    {(p.datos || []).length} partidas · {new Date(p.updated_at).toLocaleDateString("es-CL")}
                  </div>
                </button>
              ))}
            </div>
